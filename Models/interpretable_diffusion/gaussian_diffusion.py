...
class Diffusion_TS(nn.Module):
    ...
    def output(self, x, x_mark, t, padding_masks=None):
        # print(x.shape)  print(x_mark.shape) 128,24,7   128,24,4   B,L,C   
        N = 3
        B,L,C = x.shape
        if x_mark is not None:
         #---------------------------------改动1,Stock_2有较大提高  ，x_mark为时间戳,128,24,4 
            # B,L,C = x.shape
            # project1 = wn(nn.Linear(L,self.d_model)).to(x.device)
            # project2 = wn(nn.Linear(L,self.d_model)).to(x.device)
            # proj = nn.Linear(4*L, 2*L).to(x.device)
            # x_mark = x_mark.view(B, -1)           # flatten to (128, 96)
            # x_mark = proj(x_mark)              #  (128, 48)
            # # print(x.shape,x_mark.shape)
            # kbn = KBasisNet(input_len=1, output_len=N * (L + L)).to(x.device)
            # basis = kbn(x_mark[:,0].unsqueeze(1)).reshape(B,L+L,N)
            # basis = basis / torch.sqrt(torch.sum(basis**2,dim=1,keepdim=True)+self.epsilon)
            # raw_basis = basis[:,:L].permute(0,2,1)
            # # print(x.shape,raw_basis.shape)   torch.Size([128, 24, 7]) torch.Size([128, 3, 24])
            # x_p = x.permute(0,2,1)
            # x_p = project1(x_p)
            # basis = project2(raw_basis)  
            # # print(x_p.shape,basis.shape) # torch.Size([128, 7, 64]) torch.Size([128, 3, 64])
            # ca = channel_AutoCorrelationLayer().to(x.device)
            # out, _= ca(x_p,basis,basis)
            # # print(out.shape) # torch.Size([128, 7, 64])
            # out = out 
            # # out = out.permute(0,2,1)
            # proj = nn.Linear(self.d_model, L).to(x.device)
            # out = proj(out).permute(0,2,1) + x
            # # print(out.shape) torch.Size([128, 24, 7])
            # trend, season = self.model(out, t, padding_masks=padding_masks)  # transformer
            #---------------------------------改动1,Stock_2有较大提高 

            #---------------------------------改动2,x_mark为x,128,24,7  ett loss较大降低(可以改时间步)
            project1 = wn(nn.Linear(L,self.d_model)).to(x.device)
            project2 = wn(nn.Linear(L,self.d_model)).to(x.device)
            proj = nn.Linear(C*L, int(C/2)*L).to(x.device)
            x_mark = x.view(B, -1)
            x_mark = proj(x_mark)
            kbn = KBasisNet(input_len=1, output_len=N * (L + L)).to(x.device)
            basis = kbn(x_mark[:,0].unsqueeze(1)).reshape(B,L+L,N)
            ...
            # print(x.shape) # 2001, 24 , 7 ->  2001,24,4
            trend, season = self.model(x, t, padding_masks=padding_masks)  # transformer
    ...
    def _train_loss(self, x_start, t, x_mark=None, target=None, noise=None, padding_masks=None):
        ...
        model_out = self.output(x, x_mark, t, padding_masks)
